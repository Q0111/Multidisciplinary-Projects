...
data = pd.read_csv("Path of your 'diem_thi_thpt_2022.csv' file") # Đổi thành đỉa chỉ path mà file này nằm
warnings.filterwarnings("ignore")
print(data.head(5))
print(data.info())

#Step 2:
toan = data ["toan"].dropna()
print(len(toan))

ngu_van = data ["ngu_van"].dropna()
print (len(ngu_van))# 72648

tieng_anh = data ["ngoai_ngu"].dropna()
...
# Step 4:
plt.plot(toan.value_counts().sort_index(),color='blue', linestyle = ':', marker='.', markersize=10)
plt.grid(True)
plt.legend(['Mon Toan'])
plt.xlabel('Diem')
plt.ylabel('So luong')
...
mask = np.array(Image.open("Path of your 'map.jpg'")) # Đổi thành đỉa chỉ path mà file này nằm
